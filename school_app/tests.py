...
class BasicSetupTests(TestCase):
    ...
    def setUp(self):
        ...
        self.teacher1 = Teacher.objects.create(
            first_name="أستاذ", last_name="قدير",
            phone_number="0555123123", subject=self.subject_math
        )

        # Student
        self.student1 = Student.objects.create(
            first_name="طالب", last_name="مجتهد",
        ...
        self.student2 = Student.objects.create(
            first_name="طالبة", last_name="ذكية",
...
class RegistrationFeeContextTests(TestCase):
    def setUp(self):
        self.student1 = Student.objects.create(
            first_name="Test", last_name="StudentReg",
...
class AttendanceModelTests(TestCase):
    ...
    def test_attendance_creation_with_excused_absence(self):
        ...
        student = Student.objects.create(first_name="S", last_name="A", phone_number="011", guardian_phone="022", birth_day=1, birth_month=1, birth_year=2010, academic_level=level)
        subject = Subject.objects.create(name="Test Subject")
        teacher = Teacher.objects.create(first_name="T", last_name="A", phone_number="033", subject=subject)
...
class AttendanceApiTests(BasicSetupTests):
    def test_api_record_attendance_success_by_pk(self):
        ...
        self.assertEqual(data['student_name'], f"{self.student1.first_name} {self.student1.last_name}")
...
class TeacherMonthlyPaymentPageTests(BasicSetupTests):
    ...
    def test_get_teacher_monthly_payment_page_no_group_or_price(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, f"الدفع الشهري للمدرس: {self.teacher1.first_name} {self.teacher1.last_name}")
        self.assertIn('taught_groups', response.context)
        self.assertIsNone(response.context.get('selected_group'))
        self.assertEqual(response.context.get('teacher_price_per_session_str'), '')
    ...
    def test_get_teacher_monthly_payment_page_with_group_and_price(self):
        ...
        self.assertEqual(len(sessions_display), 3)
        ...
        self.assertEqual(s2_info['attended_students_count'], 1) # student2 present
        self.assertEqual(len(s2_info['absent_students']), 1) # student1 absent
        self.assertEqual(s2_info['absent_students'][0]['id'], self.student1.id)
    ...
    def test_post_calculate_payment_teacher_monthly_payment(self):
        ...
        payload = {
            ...
            'teacher_price_per_session_hidden': price_str,
            'sessions_to_pay_ids': [self.session1_g1.id, self.session2_g1.id],
            f'counted_absence_for_session_{self.session2_g1.id}_student_{self.student1.id}': 'on' # Count student1's absence in session2
        }
        response = self.client.post(reverse('teacher_monthly_payment', args=[self.teacher1.id]), data=payload)
        self.assertEqual(response.status_code, 200) # Renders page with calculation
        self.assertIn('calculated_payment_details', response.context)

        details = response.context['calculated_payment_details']
        self.assertEqual(details['num_actual_sessions'], 2) # session1_g1, session2_g1
        self.assertEqual(details['total_counted_absences'], 1) # student1 in session2_g1
        self.assertEqual(details['final_effective_sessions'], 3) # 2 actual + 1 counted absence
        self.assertEqual(details['teacher_price_per_session'], Decimal(price_str))
        self.assertEqual(details['total_amount_due'], 3 * Decimal(price_str))
        self.assertIn('sessions_paid_ids_json', details)
        self.assertIn('counted_absences_data_json', details)
    ...
    def test_post_process_payment_teacher_monthly_payment(self):
        ...
        payload_for_processing = {
            ...
            'group_id_hidden': self.group1.id,
            'teacher_price_per_session_hidden': str(price),
            'sessions_paid_ids_json': json.dumps(sessions_to_pay_ids),
        }

        response = self.client.post(reverse('teacher_monthly_payment', args=[self.teacher1.id]), data=payload_for_processing)
        self.assertEqual(response.status_code, 302)

        expected_redirect_url = f"{reverse('teacher_monthly_payment', args=[self.teacher1.id])}?group_id={self.group1.id}&teacher_price_per_session={str(price)}"
        ...
        # Fetch the redirected page to check messages
        response_redirected = self.client.get(expected_redirect_url)
        messages_list = list(response_redirected.context['messages'])
        self.assertTrue(any("تم تسجيل دفع مستحقات لـ 2 حصة بنجاح" in str(msg) for msg in messages_list))
    ...
    def test_process_payment_no_sessions_selected(self):
        ...
        payload = {
            ...
            'group_id_hidden': self.group1.id,
            'teacher_price_per_session_hidden': price_str,
            'sessions_paid_ids_json': json.dumps([]),
        }
        response = self.client.post(reverse('teacher_monthly_payment', args=[self.teacher1.id]), data=payload)
        self.assertEqual(response.status_code, 302)

        response_redirect = self.client.get(response.url)
        messages_list = list(response_redirect.context['messages'])
        self.assertTrue(any("لا توجد حصص محددة لتأكيد الدفع" in str(msg) for msg in messages_list))
    ...
    def test_calculate_payment_no_sessions_selected(self):
        ...
        self.assertTrue(any("الرجاء اختيار حصة واحدة على الأقل للدفع" in str(msg) for msg in messages_list))
        self.assertIsNone(response.context.get('calculated_payment_details'))
...
class PaymentReportPageTests(BasicSetupTests):
    ...
    def test_payment_report_custom_date_filter(self):
        ...
        new_student = Student.objects.create(
            first_name="Filt", last_name="Student", phone_number="0123", guardian_phone="0124",
            birth_day=1,birth_month=1,birth_year=2000, academic_level=self.level_high1,
            registration_fee_paid=True, created_at=filter_start_date + timezone.timedelta(days=1) # Within range
        )
...
class StudentMonthlyPaymentPageTests(BasicSetupTests):
    def test_get_student_monthly_payment_page_no_group(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, f"الدفع الشهري للطالب: {self.student1.first_name} {self.student1.last_name}")
    ...
    def test_get_student_monthly_payment_page_with_group(self):
        ...
        self.assertIn('amount_due', response.context)
        ...
        self.assertEqual(response.context['amount_due'], expected_amount_due)
    ...
    def test_post_mark_excused_student_monthly_payment(self):
        ...
        self.assertEqual(response_after_excuse.context['amount_due'], expected_amount_due_after_excuse)
    ...
    def test_post_process_payment_student_monthly_payment_exact_amount(self):
        ...
        self.assertEqual(response_after_payment.context['amount_due'], Decimal('0.00'))
    ...
    def test_post_process_payment_student_monthly_payment_overpayment(self):
        ...
        self.assertEqual(response.status_code, 302)
        messages_list = list(response.context['messages'])
        self.assertTrue(any("المبلغ المتبقي" in str(msg) and "سيتم اعتباره كرصيد" in str(msg) for msg in messages_list))
    ...
    def test_post_process_payment_student_monthly_payment_partial_payment(self):
        ...
        self.assertEqual(response_after_payment.context['amount_due'], expected_amount_due_after_partial)
    ...
    def test_post_process_payment_insufficient_for_one_session(self):
        ...
        self.assertEqual(response.status_code, 302)
        messages_list = list(response.context['messages'])
        self.assertTrue(any(f"المبلغ المتبقي {amount_to_pay} غير كافٍ لدفع حصة كاملة" in str(msg) for msg in messages_list))
...
class AttendanceApiTests(BasicSetupTests): # This line was part of the original search block for the previous incorrect diff.
    ...
    def test_api_record_attendance_missing_data(self):
        ...
        data = response.json()
        self.assertEqual(data['status'], 'error')
        self.assertEqual(data['message'], 'Session ID and Student Identifier are required.')
    ...
    def test_api_record_attendance_numeric_string_card_number_lookup(self):
        ...
        numeric_card_student = Student.objects.create(
            first_name="NumericCard", last_name="Student",
